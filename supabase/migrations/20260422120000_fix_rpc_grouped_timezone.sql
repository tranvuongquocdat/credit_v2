-- Fix timezone day-bucketing cho 5 RPC grouped.
-- Lý do: RPC cũ cast `created_at::date` dùng session TZ (UTC trên Supabase).
-- Các giao dịch 17:00-23:59 UTC (tức 00:00-06:59 VN ngày hôm sau) bị gom nhóm
-- sai ngày so với logic client cũ (partition theo giờ VN của browser).
-- Fix: cast qua Asia/Ho_Chi_Minh trước khi ::date.
...
BEGIN
  ...
  SELECT
    c.contract_code,
    (ch.created_at AT TIME ZONE 'Asia/Ho_Chi_Minh')::date AS transaction_date,
  ...
  GROUP BY
    c.contract_code,
    (ch.created_at AT TIME ZONE 'Asia/Ho_Chi_Minh')::date,
    ch.transaction_type,
    ch.is_deleted,
    cust.name,
    prof.username
...
BEGIN
  ...
  SELECT
    i.contract_code,
    (ih.created_at AT TIME ZONE 'Asia/Ho_Chi_Minh')::date AS txn_date,
...
BEGIN
  ...
  SELECT
    p.contract_code,
    (ph.created_at AT TIME ZONE 'Asia/Ho_Chi_Minh')::date AS transaction_date,
...
BEGIN
  RETURN QUERY
  SELECT
    (sfh.created_at AT TIME ZONE 'Asia/Ho_Chi_Minh')::date AS transaction_date,
...
BEGIN
  RETURN QUERY
  SELECT
    (t.created_at AT TIME ZONE 'Asia/Ho_Chi_Minh')::date AS transaction_date,
