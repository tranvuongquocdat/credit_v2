...
/**
 * Lấy lịch sử thanh toán lãi phí của một hợp đồng pawn trong khoảng thời gian
 * @param pawnId - ID của hợp đồng pawn
 * @param startDate - Ngày bắt đầu (YYYY-MM-DD)
 * @param endDate - Ngày kết thúc (YYYY-MM-DD)
 * @returns Promise<PaymentHistoryRecord[]> - Danh sách lịch sử thanh toán trong khoảng thời gian
 */
export async function getPawnPaymentHistoryByDateRange(
  pawnId: string, 
  startDate: string, 
  endDate: string
): Promise<PaymentHistoryRecord[]> {
 ...
 */
export async function getPawnPaymentHistoryByStatus(
  pawnId: string, 
...
/**
 * Lấy tổng số tiền đã thanh toán của một hợp đồng pawn
 * @param pawnId - ID của hợp đồng pawn
 * @returns Promise<number> - Tổng số tiền đã thanh toán
 */
export async function getTotalPaidAmount(pawnId: string): Promise<number> {
  const { data, error } = await supabase
    .from('pawn_history')
  ...
  return data.reduce((total, record) => {
    const pawnAmount = record.credit_amount || 0;
    const debitAmount = record.debit_amount || 0;
    return total + pawnAmount - debitAmount;
  }, 0);
}
 ...
 */
export async function getPaymentPeriods(pawnId: string): Promise<Array<{
  start_date: string;
  end_date: string;
  total_amount: number;
  period_number?: number;
}>> {
  const paymentHistory = await getPawnPaymentHistory(pawnId);
 ...
 */
export async function hasPaymentOnDate(pawnId: string, date: string): Promise<boolean> {
  const { data, error } = await supabase
    .from('pawn_history')
    .select('id')
    .eq('pawn_id', pawnId)
