...
export default function EditCreditPage({ params }: { params: { id: string } }) {
  ...
  // Load thông tin hợp đồng và danh sách khách hàng
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const { data: creditData, error: creditError } = await getCreditById(id);
        ...
        // Set giá trị mặc định cho form từ dữ liệu hợp đồng
        form.reset({
          customer_id: creditData.customer_id,
          contract_code: creditData.contract_code || '',
          id_number: creditData.id_number || '',
          phone: creditData.phone || '',
          address: creditData.address || '',
          collateral: creditData.collateral || '',
          loan_amount: creditData.loan_amount,
          interest_type: creditData.interest_type,
          interest_value: creditData.interest_value,
          loan_period: creditData.loan_period,
          interest_period: creditData.interest_period,
          loan_date: new Date(creditData.loan_date),
          notes: creditData.notes || '',
          status: creditData.status as CreditStatus || CreditStatus.ON_TIME,
          store_id: creditData.store_id,
        });
      } catch (err) {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      // Gọi API cập nhật hợp đồng
      const { data, error } = await updateCredit(id, updateData);
  ...
  return (
    <Layout>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Thông tin hợp đồng #{credit?.contract_code || id}</CardTitle>
          <CardDescription>
