      ...
      {enhancedCredits.map((credit, index) => (
        <div key={credit.id} className="bg-white border rounded-lg p-3 shadow-sm">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mb-2 text-xs text-gray-600">
            <span><span className="text-gray-500">Mã HĐ:</span> <span className="font-medium text-gray-700">{credit.contract_code}</span></span>
            {credit.customer?.phone && (
              <span><span className="text-gray-500">SĐT:</span> <a href={`tel:${credit.customer.phone}`} className="font-medium text-blue-600">{credit.customer.phone}</a></span>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-2 text-xs">
            <div>
              ...
              <div className="font-medium text-gray-800">{formatCurrency(credit.loan_amount || 0)}</div>
            </div>
            <div>
              <div className="text-gray-500">Tổng lãi</div>
              <div className="font-medium text-gray-800">{formatCurrency(credit.totalInterest || 0)}</div>
            </div>
            <div>
              <div className="text-gray-500">Tổng tiền</div>
              <div className="font-bold text-red-600">{formatCurrency((credit.loan_amount || 0) + (credit.totalInterest || 0))}</div>
            </div>
          </div>
          ...
          <div className="pt-2 border-t text-xs">
            ...
            <span className="text-orange-600 font-medium">{credit.reason}</span>
          </div>
        </div>
      ))}

      {/* Totals card */}
      {enhancedCredits.length > 0 && (
        <div className="bg-yellow-100 border border-yellow-300 rounded-lg p-3">
          <h3 className="font-bold text-center mb-2 text-sm">Tổng kết ({enhancedCredits.length} hợp đồng)</h3>
          <div className="grid grid-cols-3 gap-2 text-xs">
            ...
            <div className="text-center">
              <div className="text-gray-600">Tổng lãi</div>
